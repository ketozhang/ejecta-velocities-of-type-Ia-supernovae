...
def lnlike(params, v):
    ...
    return -0.5 * np.sum(
        ((v - v_hat) / scatter) ** 2 + np.log(2 * np.pi * scatter ** 2)
    )
...
if __name__ == "__main__":
    ...
    ndim, nwalkers = 6, 100
    pos = [
        np.random.normal(param, param / 2, nwalkers)
        for param in np.append(BIMODAL_PARAMS, 1)
    ]
    pos = np.array(pos).T

    sampler = EnsembleSampler(
        nwalkers, ndim, lnprob, args=(np.arange(len(v)), v)
    )
